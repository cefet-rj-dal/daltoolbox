...
2. minerar itemsets e regras;
3. reduzir redundância e priorizar padrões úteis;
4. avançar para sequências e grafos.
Em cada etapa, compare sempre volume de padrões versus interpretabilidade.

## Configuração

Nesta abertura, carregamos pacotes para mineração, visualização e variantes estruturadas (sequências/grafos).
...
Antes de minerar, inspecionamos dimensão e amostra dos dados para entender cardinalidade e tipos de atributos.
...
Começamos com limpeza mínima para evitar que ruído estrutural distorça suporte e confiança.
...
Na sequência, aplicamos hierarquia conceitual e binning para elevar interpretabilidade sem perder padrão global.
...
Leitura recomendada: filtre primeiro por suporte/confianca e depois priorize por lift.
...
Converter para `data.frame` facilita ranking, filtro e comparação de regras.
...
Aqui vale comparar com Apriori: ECLAT foca itemsets; Apriori já pode produzir regras.
...
Esses dois conjuntos ajudam a compactar resultados sem perder o sinal principal.
...
No bloco seguinte, ordenamos por lift para destacar associações potencialmente mais informativas.
...
Primeiro avaliamos cobertura de uma regra; depois ampliamos para múltiplas regras.
...
Use os dois gráficos de forma complementar: visão global e visão relacional (paracoord).
...
Esta etapa é útil para detectar exceções e comportamentos menos frequentes.
...
Aqui adicionamos uma dimensão semântica (`edu_level`) para extrair regras mais interpretáveis por perfil.
...
Diferença-chave: em sequências, a ordem dos eventos influencia o padrão encontrado.
...
Com os dados carregados, executamos o SPADE para descobrir padrões sequenciais frequentes.
...
## Mineração de Padrões em Grafos (Exemplo)
Em grafos, padrões frequentes são subgrafos recorrentes. Aqui mostramos contagem de subgrafos como ilustração.  
Slides: 63–64.

Encerramos com grafos para mostrar que o conceito de padrão frequente se estende a estruturas relacionais.
