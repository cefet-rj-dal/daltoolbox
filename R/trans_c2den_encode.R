...
#'@import reticulate
#'@export
c2den_encode <- function(input_size, encoding_size, batch_size = 32, num_epochs = 50, learning_rate = 0.001) {
  obj <- dal_transform()
  ...
  class(obj) <- append("c2den_encode", class(obj))

  return(obj)
}

#'@export
fit.c2den_encode <- function(obj, data, return_loss=FALSE, ...) {
