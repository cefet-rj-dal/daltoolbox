...
#' @description Draw a simple bar chart from a two-column data frame: first
#'   column as categories (`x`), second as values (`y`).
#' @details If `colors` is provided, a constant fill is used; otherwise
#'   ggplot2's default palette applies. `alpha` controls bar transparency. The
#'   first column is coerced to factor when needed, which makes the function
#'   convenient for quick summaries of grouped values.
#' @param data Two-column data frame: category in the first column, numeric
#'   values in the second.
#' @param label_x Label for the x-axis.
#' @param label_y Label for the y-axis.
#' @param colors Optional fill color.
#' @param alpha Bar transparency between 0 and 1.
...
plot_bar <- function(data, label_x = "", label_y = "", colors = NULL, alpha = 1) {
  ...
  grf <- grf + ggplot2::theme(legend.title = ggplot2::element_blank()) +
