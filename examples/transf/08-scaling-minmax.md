...
In practice, this is one of the most common preprocessing steps before distance-based or scale-sensitive learners. The value of the example is not only the call to `minmax()`, but the before/after comparison.
...
What to observe
- Numeric attributes that originally had different ranges are now placed on a common scale.
- The categorical target remains untouched, which is what you usually want in supervised tasks.
...
Common mistakes
- Fitting the normalization object on the full dataset before the train/test split.
- Expecting normalization to improve every model equally.
- Forgetting that interpretability changes when the variables are shown only in the transformed scale.
